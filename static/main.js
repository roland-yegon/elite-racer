...
let currentGear = 1; // -1: Reverse, 0: Neutral, 1-6: Forward
...
function initControls() {
  window.addEventListener("keydown", (e) => {
    ...
    if (key === "r") {
      ...
      vehicle.chassisBody.quaternion.setFromEuler(0, euler.y, 0);
      vehicle.chassisBody.velocity.set(0, 0, 0);
      vehicle.chassisBody.angularVelocity.set(0, 0, 0);
    }
    if (key === "m") {
      isManualGear = !isManualGear;
      document.getElementById("gear-mode-text").innerText = isManualGear
        ? "Manual"
        : "Auto";
    }
    if (isManualGear) {
...
function animate() {
  ...
  const speedKmH = Math.abs(Math.round(v.length() * 3.6));

  if (!isManualGear) {
    if (forwardSpeed > 2) {
      if (speedKmH < 40) currentGear = 1;
      else if (speedKmH < 80) currentGear = 2;
      else if (speedKmH < 130) currentGear = 3;
      else if (speedKmH < 180) currentGear = 4;
      else if (speedKmH < 230) currentGear = 5;
      else currentGear = 6;
    } else if (forwardSpeed < -2) {
      currentGear = -1;
    } else {
      if (isBackward) currentGear = -1;
      else if (isForward) currentGear = 1;
      else currentGear = 1;
    }
  }

  const gearForces = [0, 4500, 3500, 2500, 1800, 1200, 800];
  let maxEngineForce = 0;
  if (currentGear > 0) maxEngineForce = gearForces[currentGear];
  else if (currentGear === -1) maxEngineForce = 3500;
  ...
  if (isBackward) {
    if (currentGear === -1) {
      engineForce = -maxEngineForce / 2; // reverse slower
  ...
  switch (cameraMode) {
    case 0:
      relativeCameraOffset = new THREE.Vector3(0, 4, 10);
      break; // Chase High
    case 1:
      relativeCameraOffset = new THREE.Vector3(0, 2, 7);
      break; // Chase Low
    case 2:
      relativeCameraOffset = new THREE.Vector3(0, 1, -0.6);
      break; // Hood Cam
    case 3:
      relativeCameraOffset = new THREE.Vector3(0, 20, 2);
      break; // Top-Down
  }

  const euler = new THREE.Euler().setFromQuaternion(
    chassisVisual.quaternion,
    "YXZ",
  );
  const yaw = euler.y;
  const finalYaw = keys["b"] ? yaw + Math.PI : yaw;
  ...
  const cameraTargetMatrixWorld = new THREE.Matrix4()
    .compose(chassisVisual.position, yawQuat, new THREE.Vector3(1, 1, 1))
    .multiply(cameraTargetMatrix);

  const targetPos = new THREE.Vector3();
  targetPos.setFromMatrixPosition(cameraTargetMatrixWorld);

  camera.position.lerp(targetPos, cameraMode === 2 || keys["b"] ? 0.8 : dt * 5);

  const lookAtTarget = chassisVisual.position.clone();
  if (cameraMode === 2) {
    // Hood Cam looks straight ahead
  ...
  const speedDisplay = Math.round(velocity * 3.6);
  document.getElementById("speed-value").innerText = speedDisplay;

  let displayGear =
    currentGear > 0 ? currentGear : currentGear === 0 ? "N" : "R";
  document.getElementById("gear-value").innerText = displayGear;
